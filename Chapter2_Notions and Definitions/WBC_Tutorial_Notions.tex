...
\begin{itemize}
\item \textbf{Inaccuracy.} For example, Chow \textit{et al.} proposed \textcolor{red}{local security} to describe the ambiguity of keyed LUT. But this property might be insufficient for the security of a WBC implementation.

\item \textbf{Misappropriation.} \textcolor{red}{code lifting} is a requirement of high-level application based on WBC, e.g., DRM applications require the binary codes are device-binding. It might not be self-contented by a WBC algorithm, but a combined security property which is supported by algorithm design, implementation, code obfuscation and trusted computing base.
...
\begin{itemize}
\item \textcolor{red}{\textbf{Black-Box Primitive Security.}} For WBC security notions, first we need to consist with the security framework of its black-box securities, such as differential and linear cryptanalysis resistance. It is obvious that the security notions on the primitive of a WBC, e.g., AES security in the Chow \textit{et al.} WBAES, definitely should be considered.
 
\item \textcolor{red}{\textbf{White-Box Implementation Security.}} For a WBC implementation, it should obey its security requirements which are defined in the white-box context. For example, the white-box key recovery attacks, the differential fault analysis, and so on.


\item \textcolor{red}{\textbf{White-Box Application Security.}} Even a WBC implementation is secure on both of its black-box primitive and carefully implemented in the application. It might be still not provide enough security for certain applications. E.g., the space-hardness proposals \textcolor{blue}{SPACE} and \textcolor{blue}{SPNBox} are proven secure under the white-box security model. But they cannot provide code-lifting protection which is necessary in DRM applications.
\end{itemize}
}
